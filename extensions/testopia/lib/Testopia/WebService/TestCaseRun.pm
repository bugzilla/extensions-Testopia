...
sub get {
    ...
    if ($build_id && $build_id !~ /^\d+$/){ 
        ...
        my $build = Testopia::Build::check_build($build_id, $run->product, "THROW");
        $build_id = $build->id;
    }
    if ($env_id && $env_id !~ /^\d+$/){ 
        my $run = Testopia::TestRun->new($run_id);
        ThrowUserError('invalid-test-id-non-existent') unless $run;
        my $environment = Testopia::Build::check_environment($env_id, $run->product, "THROW");
        $env_id = $environment->id;
    }
    #Result is a test case run hash map
    my $caserun = new Testopia::TestCaseRun($run_id, $case_id, $build_id, $env_id);
...
sub list {
    ...
    return Testopia::Table->new('case_run','tr_xmlrpc.cgi',$cgi,undef,$search->query())->list();
}
...
sub list_count {
    ...
    my $search = Testopia::Search->new($cgi);
    return Testopia::Table->new('case_run','tr_xmlrpc.cgi',$cgi,undef,$search->query())->list_count();
}
...
sub create {
    ...
    my $run = Testopia::TestRun->new($new_values->{'run_id'});
    ...
    my $caserun = Testopia::TestCaseRun->create($new_values);
...
sub update {
    ...
    my @caseruns;
    my @ids = Testopia::Util::process_list($run_id);
    if (ref $case_id eq 'HASH' && !$build_id){
        $new_values = $case_id;
        foreach my $id (@ids){
            my $caserun = new Testopia::TestCaseRun($id);
            if ($caserun){
...
sub lookup_status_id_by_name {
    ...
    return Testopia::TestCaseRun::lookup_status_by_name($name);
}
...
sub lookup_status_name_by_id {
    ...
    return Testopia::TestCaseRun::lookup_status($id);
}
...
=item C<get($caserun_id)>
 ...
 Returns:     A blessed Testopia::TestCaseRun object hash

...
sub parse() {
    ...
    foreach my $twig_category ( $root->children('category') ) {
        ...
        if ( $category_name eq "" ) {
            $self->error(
                "Category name cannot be empty, product='" . $product_name . "', description='" . $description . "'." );
            next;
        }

        $description = "FIX ME.  Created during category import with no description supplied."
          if ( $description eq "" );
    ...
    foreach my $twig_testplan ( $root->children('testplan') ) {
        ...
        if ( !$author_ref->[0] ) {
            $self->error(
                "Cannot find author '" . $author . "' in test plan '" . $twig_testplan->field('name') . "'." );
        }
        else {
        ...
        if ( !defined($product_id) ) {
            $self->error( "Cannot find product '"
                  . $twig_testplan->field('product')
                  . "' in test plan '"
                  . $twig_testplan->field('name')
                  . "'." );
        }
        ...
        $self->error( "Length of Test Plan name '" 
              . $name
              . "' must be "
              . Testopia::TestPlan->NAME_MAX_LENGTH
              . " characters or less." )
          if ( defined($name) && ( length($name) > Testopia::TestPlan->NAME_MAX_LENGTH ) );
        ...
        foreach my $twig_attachments (@attachments) {
            ...
            my $submitter_id = -1;
            if ( !$submitter_ref->[0] ) {
                $self->error( "Cannot find submitter '"
                      . $submitter
                      . "' in test plan '"
                      . $twig_testplan->field('name')
                      . "' attachment '"
                      . $twig_attachments->field('description')
                      . "'." );
            }
            else {
    ...
    foreach my $twig_testcase ( $root->children('testcase') ) {
        ...
        $self->error( "Length of summary '" 
              . $summary
              . "' must be "
              . Testopia::TestCase->SUMMARY_MAX_LENGTH
              . " characters or less." )
          if ( defined($summary) && ( length($summary) > Testopia::TestCase->SUMMARY_MAX_LENGTH ) );
        ...
        $self->error( "Cannot find status '" . $twig_testcase->att('status') . "' in test case '" . $summary . "'." )
        ...
        $self->error( "Length of alias '" 
              . $alias
              . "' in test case '"
              . $summary
              . "' must be "
              . Testopia::TestCase->ALIAS_MAX_LENGTH
              . " characters or less." )
          if ( defined($alias) && ( length($alias) > Testopia::TestCase->ALIAS_MAX_LENGTH ) );
        my $requirement = entity_replace_xml( $twig_testcase->field('requirement'), STRIP_BOTH ) || undef;
        $self->error( "Length of requirement '"
              . $requirement
              . "' in test case '"
              . $summary
              . "' must be "
              . Testopia::TestCase->REQUIREMENT_MAX_LENGTH
              . " characters or less." )
          if ( defined($requirement) && ( length($requirement) > Testopia::TestCase->REQUIREMENT_MAX_LENGTH ) );
        ...
        foreach my $twig_testplan_reference ( $twig_testcase->children(TESTPLAN_REFERENCE) ) {
            ...
            if ( $testplan_reference eq "" ) {
                ...
                !$xml_testcase->testplan->add(
                    $twig_testplan_reference->att('type'),
                    entity_replace_xml( $testplan_reference, STRIP_BOTH )
                )
              )
            {
                $self->error( "Do not know how to handle test plan of type '"
                      ...
                      . $twig_testcase->field('summary') . "'."
                      . "\nKnown types are: ("
                      . uc XMLREFERENCES_FIELDS
                      . ")." );
            }
        }

        # Keep track of this testcase's alias.  Used during verification to insure aliases are unique.
        $self->testcase_aliases( entity_replace_xml( $twig_testcase->field('summary'), STRIP_BOTH ), $alias )
          if ( defined $alias );
        ...
        foreach my $twig_tag (@tags) {
            ...
            $self->error( "Length of tag '" 
                  . $tag
                  . "' in test case '"
                  . $summary
                  . "' must be "
                  . Testopia::TestCase->TAG_MAX_LENGTH
                  . " characters or less." )
              if ( defined($tag) && ( length($tag) > Testopia::TestCase->TAG_MAX_LENGTH ) );
        ...
        foreach my $twig_component (@components) {
            my $results =
              $xml_testcase->add_component( entity_replace_xml( $twig_component->children_text(PCDATA), STRIP_BOTH ),
                $twig_component->att('product') );
            $self->error($results) if ( $results ne "" );
        }
        foreach my $twig_blocks ( $twig_testcase->children(BLOCKS) ) {
            if (
                !$xml_testcase->blocks->add(
                    $twig_blocks->att('type'),
                    entity_replace_xml( $twig_blocks->children_text(PCDATA), STRIP_BOTH )
                )
              )
            {
                $self->error( "Do not know how to handle a blocking test case of type '"
                      . $twig_blocks->att('type')
                      . "' in test case '"
                      . $xml_testcase->testcase->summary()
                      . "'." );
            }
        }
        ...
        foreach my $twig_dependson ( $twig_testcase->children(DEPENDSON) ) {
            if (
                !$xml_testcase->dependson->add(
                    $twig_dependson->att('type'),
                    entity_replace_xml( $twig_dependson->children_text(PCDATA), STRIP_BOTH )
                )
              )
            {
                $self->error( "Do not know how to handle dependency of type '"
                      . $twig_dependson->att('type')
                      . "' in test case '"
                      . entity_replace_xml( $xml_testcase->testcase->summary(), STRIP_BOTH )
                      . "'." );
            }
        }
    }
    ...
    foreach my $summary ( keys %{ $self->testcase_aliases } ) {
        ...
        if ( exists $duplicate_alias{$alias} ) {
            ...
            if ( $duplicate_alias{$alias} ne "import" ) {
                $error_message .=
                  "  Test Case " . $duplicate_alias{$alias} . " already uses the alias '" . $alias . "'.";
            }
            else {

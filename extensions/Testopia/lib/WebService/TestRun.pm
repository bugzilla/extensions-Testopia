...
package Bugzilla::Extension::Testopia::WebService::TestRun;
...
sub list {
    ...
    return Bugzilla::Extension::Testopia::Table->new('run','tr_xmlrpc.cgi',$cgi,undef,$search->query())->list();
}
...
sub list_count {
    ...
    my $search = Bugzilla::Extension::Testopia::Search->new($cgi);
    return Bugzilla::Extension::Testopia::Table->new('run','tr_xmlrpc.cgi',$cgi,undef,$search->query())->list_count();
}
...
sub create {
    ...
    my $plan = Bugzilla::Extension::Testopia::TestPlan->new($new_values->{'plan_id'});
    ...
    if (trim($new_values->{'build_id'}) !~ /^\d+$/ ){
        my $build = Bugzilla::Extension::Testopia::Build::check_build($new_values->{'build_id'}, $plan->product, "THROWERROR");
        $new_values->{'build_id'} = $build->id;
    }
    if (trim($new_values->{'environment_id'}) !~ /^\d+$/ ){
        my $environment = Bugzilla::Extension::Testopia::Environment::check_environment($new_values->{'environment_id'}, $plan->product, "THROWERROR");
...
sub update {
    ...
    $timestamp = Bugzilla::Extension::Testopia::Util::get_time_stamp() if $new_values->{'status'} == 0 && !$run->stop_date;
...
sub get_completion_report {
    ...
    foreach my $g (@run_ids){
        my $obj = Bugzilla::Extension::Testopia::TestRun->new($g);
...
L<Bugzilla::Extension::Testopia::TestRun>

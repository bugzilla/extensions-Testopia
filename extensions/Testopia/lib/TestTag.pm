...
Bugzilla::Extension::Testopia::TestTag - A Testopia tag
...
use Bugzilla::Extension::Testopia::TestTag;
 ...
 $tag = Bugzilla::Extension::Testopia::TestTag->new($tag_id);
 $tag = Bugzilla::Extension::Testopia::TestTag->new($tag_hash);

=cut

package Bugzilla::Extension::Testopia::TestTag;
...
sub check_tag {
    ...
    my ($id) = $dbh->selectrow_array(
            "SELECT tag_id FROM test_tags
              ...
              undef, lc($name));
    if ($id){
        return Bugzilla::Extension::Testopia::TestTag->new($id);
    }
    else{
        return undef;
    }

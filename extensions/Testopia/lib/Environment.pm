...
Bugzilla::Extension::Testopia::Environment - A test environment
...
=head1 SYNOPSIS
 ...
 $env = Bugzilla::Extension::Testopia::Environment->new($env_id);
 $env = Bugzilla::Extension::Testopia::Environment->new(\%env_hash);

=cut

package Bugzilla::Extension::Testopia::Environment;
...
our @EXPORT = qw(check_environment);
...
sub _check_product {
    ...
    require Bugzilla::Extension::Testopia::Product;
...
sub create_full {
    ...
    foreach my $key (keys(%{$environment})){
        require Bugzilla::Extension::Testopia::Environment::Category;
        my $cat = Bugzilla::Extension::Testopia::Environment::Category->new({'product_id' => $prod_id});
...
# internal function used by create_full()
    ...
    foreach my $key (keys(%{$hash})) {
        if(ref($hash->{$key})){ # must be element, since property contains value instead of another hash
            require Bugzilla::Extension::Testopia::Environment::Element;
            ...
            if(!$prop_id){
                ...
                $prop = Bugzilla::Extension::Testopia::Environment::Property->new($prop_id);
...
sub get_environment_elements{
    ...
    foreach my $val  (@$ref){
        push @elements, Bugzilla::Extension::Testopia::Environment::Element->new($val);
    }   
...
sub element_categories {
    ...
    my @elements;
    foreach my $val  (@$ref){
        push @elements, Bugzilla::Extension::Testopia::Environment::Category->new($val);
    }   
...
sub check_environment{
    ...
    if ($throw){
        ...
        return Bugzilla::Extension::Testopia::Environment->new($used);
    }

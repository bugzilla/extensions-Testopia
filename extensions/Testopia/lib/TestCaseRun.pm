...
package Bugzilla::Extension::Testopia::TestCaseRun;
...
use constant VALIDATORS => {
    ...
    priority_id        => \&Bugzilla::Extension::Testopia::TestCase::_check_priority,
};
...
sub _check_case_id {
    my ($invocant, $id) = @_;
    return Bugzilla::Extension::Testopia::Util::validate_test_id($id, 'case');
}

sub _check_run_id {
    my ($invocant, $id) = @_;
    return Bugzilla::Extension::Testopia::Util::validate_test_id($id, 'run');
}
...
sub _check_case_run_status_id {
    ...
    my $status_id;
    if ($status =~ /^\d+$/){
        $status_id = Bugzilla::Extension::Testopia::Util::validate_selection($status, 'case_run_status_id', 'test_case_run_status');
    }
    else {
...
sub run_create_validators {
    ...
    $params->{build_id} = Bugzilla::Extension::Testopia::TestRun::->_check_build($params->{build_id}, $run->plan->product);
    $params->{environment_id} = Bugzilla::Extension::Testopia::TestRun::->_check_env($params->{environment_id}, $run->plan->product);
...
sub create {
    ...
    my $case = Bugzilla::Extension::Testopia::TestCase->new($params->{case_id});
...
sub switch {
   ...
   if ($is){
       $self = Bugzilla::Extension::Testopia::TestCaseRun->new($is);
   }
   else {
       ...
       if ($oldbuild != $build_id){
           my $build = Bugzilla::Extension::Testopia::Build->new($oldbuild);
       ...
       if ($oldenv != $env_id){
           my $environment = Bugzilla::Extension::Testopia::Environment->new($oldenv);
...
sub set_status {
    ...
    $status_id = $self->_check_case_run_status_id($status_id);
...
sub attach_bug {
    ...
    $bugs = Bugzilla::Extension::Testopia::TestCase->_check_bugs($bugs, "ATTACH");
...
sub obliterate {
    ...
    if ($single){
        ...
        if (scalar @{$ref}){
            my $cr = new Bugzilla::Extension::Testopia::TestCaseRun(@{$ref}[0]);
            $cr->set_as_current;
        }
    }
    else {
...
sub run {
    ...
    require Bugzilla::Extension::Testopia::TestRun;
    $self->{'run'} = Bugzilla::Extension::Testopia::TestRun->new($self->{'run_id'});
    return $self->{'run'};
}
...
sub case {
    ...
    require Bugzilla::Extension::Testopia::TestCase;
    $self->{'case'} = Bugzilla::Extension::Testopia::TestCase->new($self->{'case_id'});
    return $self->{'case'};
}
...
sub attachments {
    ...
    foreach my $attach (@{$attachments}){
        my $att = Bugzilla::Extension::Testopia::Attachment->new($attach->{attachment_id});
...
Bugzilla::Extension::Testopia::TestCaseRun - Testopia Test Case Run object
...
use Bugzilla::Extension::Testopia::TestCaseRun;
 ...
 $caserun = Bugzilla::Extension::Testopia::TestCaseRun->new($caserun_id);
 $caserun = Bugzilla::Extension::Testopia::TestCaseRun->new(\%caserun_hash);

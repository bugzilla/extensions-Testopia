...
package Bugzilla::Extension::Testopia::Install;
...
sub set_db_schema_abstract_schema {
    my ($schema) = @_;
...
sub testopiaUpdateDB {
    ...
    if (grep(/^test_cases$/, $dbh->bz_table_list_real) and
        ...
        foreach my $table (grep(/^test_/, $built_schema->get_table_list())) {
            $dbh->_bz_real_schema->add_table($table, $built_schema->get_table_abstract($table));
        }
...
sub updateACLs {
    ...
    foreach my $plan (@$ref) {
        my ($finished) = $dbh->selectrow_array(
          "SELECT COUNT(*) FROM test_plan_permissions WHERE plan_id = ? AND userid = ?",
          undef, ($plan->{'plan_id'}, $plan->{'author_id'}));
        next if $finished;
        $dbh->do("INSERT INTO test_plan_permissions(userid, plan_id, permissions) VALUES(?,?,?)",
...
sub migrateAttachments {
    ...
    my $rows = $dbh->selectall_arrayref(
        "SELECT attachment_id, case_id, plan_id FROM test_attachments", {'Slice' => {}});

    foreach my $row (@$rows) {
        if ($row->{'case_id'}) {
            $dbh->do("INSERT INTO test_case_attachments (attachment_id, case_id) VALUES (?,?)",
                     undef, ($row->{'attachment_id'}, $row->{'case_id'}));
        }
        elsif ($row->{'plan_id'}) {
            $dbh->do("INSERT INTO test_plan_attachments (attachment_id, plan_id) VALUES (?,?)",
                     undef, ($row->{'attachment_id'}, $row->{'plan_id'}));
        }
    }
...
sub migrateEnvData {
    my $dbh = Bugzilla->dbh;
    my ($sth, $value, $os_mapping, $platform_mapping, $ary_ref, $i);
    ...
    $sth = $dbh->prepare("INSERT INTO test_environment_map " .
        ...
        "VALUES (?, ?, ?, ?)");
    $ary_ref = $dbh->selectall_arrayref("SELECT environment_id, op_sys_id FROM test_environments");
    foreach $i (@$ary_ref) {
        $sth->execute(@$i[0], 0, $os_mapping->{@$i[1]}->{'element_id'}, '');
    }
    $ary_ref = $dbh->selectall_arrayref("SELECT environment_id, rep_platform_id FROM test_environments");
    foreach $i (@$ary_ref) {
...
sub fixTables {
    ...
    eval {
        $dbh->bz_add_index('test_case_components', 'components_case_id_idx',
                           {FIELDS => [qw(case_id component_id)], TYPE => 'UNIQUE'});
    };
    if ($@) {
        ...
        foreach my $row (@$rows) {
            ...
            if (!$seen->{$line}) {
                ...
                $dbh->do("DELETE FROM test_case_components WHERE case_id = ? AND component_id = ?",
                         undef, ($row->{'case_id'}, $row->{'component_id'}));
                $dbh->do("INSERT INTO test_case_components VALUES(?,?)",
                         undef, ($row->{'case_id'}, $row->{'component_id'}));
                $seen->{$line} = 'fixed';
            }
...
sub finalFixups {
    ...
    my $count = $dbh->selectrow_array("SELECT COUNT(*) FROM test_fielddefs WHERE name = 'estimated_time'");
    unless ($count) {
        $dbh->do("INSERT INTO test_fielddefs (name, description, table_name) " .
            "VALUES ('estimated_time', 'Estimated Time', 'test_cases')");
    }

    $count = $dbh->selectrow_array("SELECT COUNT(*) FROM test_case_runs WHERE priority_id = 0");
    if ($count) {
        ...
        $dbh->do("UPDATE test_case_runs SET priority_id = (SELECT id FROM priority WHERE value = ?) 

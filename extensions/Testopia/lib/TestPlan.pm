...
package Bugzilla::Extension::Testopia::TestPlan;
...
our @EXPORT = qw(lookup_type_by_name lookup_type);
...
sub _check_product {
    ...
    if ($product_id !~ /^\d+$/ ){
        ...
        $product = Bugzilla::Extension::Testopia::Product->new($product_id);
    }
...
sub _check_type {
    ...
    Bugzilla::Extension::Testopia::Util::validate_selection($type_id, 'type_id', 'test_plan_types');
    return $type_id;
}
...
sub update {
    ...
    foreach my $field (keys %$changed){
        Bugzilla::Extension::Testopia::Util::log_activity('plan', $self->id, $field, $timestamp, $changed->{$field}->[0], $changed->{$field}->[1]);
    }
...
Returns a reference to a list of Bugzilla::Extension::Testopia::TestCase objects linked
to this plan
...
sub test_cases {
    ...
    foreach my $id (@{$caseids}){
        push @cases, Bugzilla::Extension::Testopia::TestCase->new($id);
    }
...
sub builds_seen {
    ...
    require Bugzilla::Extension::Testopia::Build;
    ...
    my @o;      
    foreach my $id (@$ref){
        push @o, Bugzilla::Extension::Testopia::Build->new($id);
    }
    return \@o;
}
...
sub environments_seen {
    ...
    require Bugzilla::Extension::Testopia::Environment;
    ...
    my @o; 
    foreach my $id (@$ref){
        push @o, Bugzilla::Extension::Testopia::Environment->new($id);
    }
    return \@o;   
}
...
sub tags {
    ...
    my @plan_tags;
    foreach my $t (@{$tagids}){
        push @plan_tags, Bugzilla::Extension::Testopia::TestTag->new($t);
    }
    $self->{'tags'} = \@plan_tags;
    return $self->{'tags'};
}
...
Bugzilla::Extension::Testopia::TestPlan - Testopia Test Plan object
...
use Bugzilla::Extension::Testopia::TestPlan;
 ...
 $plan = Bugzilla::Extension::Testopia::TestPlan->new($plan_id);
 $plan = Bugzilla::Extension::Testopia::TestPlan->new({});

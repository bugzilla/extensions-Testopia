...
package Bugzilla::Extension::Testopia::TestRun;
...
our @EXPORT = qw(calculate_percent);
...
sub _check_plan {
    ...
    Bugzilla::Extension::Testopia::Util::validate_test_id($plan_id, 'plan');
    return $plan_id;
}
...
sub _check_env{
    ...
    if (ref $environment){
        ...
        $environment_id = Bugzilla::Extension::Testopia::Util::validate_selection($environment, 'environment_id', 'test_environments');
    }
    else {
        $environment_id = Bugzilla::Extension::Testopia::Environment::check_environment($environment, $product);
    }
...
sub _check_build{
    ...
    if (ref $build){
        ...
        $build_id = Bugzilla::Extension::Testopia::Util::validate_selection($build, 'build_id', 'test_builds');
    }
    else {
        $build_id = Bugzilla::Extension::Testopia::Build::check_build($build, $product);
    }
...
sub run_create_validators {
    ...
    my $plan = Bugzilla::Extension::Testopia::TestPlan->new($params->{plan_id});
...
sub create {
    my ($class, $params) = @_;
    require Bugzilla::Extension::Testopia::TestPlan;
...
sub update {
    ...
    foreach my $field (keys %$changed){
        Bugzilla::Extension::Testopia::Util::log_activity('run', $self->id, $field, $timestamp, $changed->{$field}->[0], $changed->{$field}->[1]);
    }
...
sub add_case_run {
    ...
    my $caserun = Bugzilla::Extension::Testopia::TestCaseRun->create({
        'run_id'     => $self->{'run_id'},
        'case_id'    => $case_id,
        'assignee'   => $assignee,
...
sub filter_environments {
    ...
    my @environments;
    foreach my $id (@$ids){
        push @environments, Bugzilla::Extension::Testopia::Environment->new($id);
    }
    return \@environments;
}
...
sub filter_builds {
    ...
    my @builds;
    foreach my $id (@$ids){
        push @builds, Bugzilla::Extension::Testopia::Build->new($id);
    }
    return \@builds;
}
...
Returns a reference to a list of Bugzilla::Extension::Testopia::Environment objects.
...
Returns the Bugzilla::Extension::Testopia::TestPlan object of the plan this run 
is assoceated with
...
sub plan {
    ...
    require Bugzilla::Extension::Testopia::TestPlan;
    $self->{'plan'} = Bugzilla::Extension::Testopia::TestPlan->new($self->{'plan_id'});
    return $self->{'plan'};
}
...
Returns the Bugzilla::Extension::Testopia::Environment object of the environment 
...
sub environment {
    ...
    $self->{'environment'} = Bugzilla::Extension::Testopia::Environment->new($self->{'environment_id'});
...
Returns the Bugzilla::Extension::Testopia::Build object of the plan this run 
is assoceated with
...
sub build {
    ...
    $self->{'build'} = Bugzilla::Extension::Testopia::Build->new($self->{'build_id'});
    return $self->{'build'};
...
sub cases {
    ...
    foreach my $cr (@{$self->current_caseruns}){
        push @cases, Bugzilla::Extension::Testopia::TestCase->new($cr->case_id);
    }
    $self->{'cases'} = \@cases;
    return $self->{'cases'};
...
sub current_caseruns {
    ...
    foreach my $id (@{$ref}){
        push @caseruns, Bugzilla::Extension::Testopia::TestCaseRun->new($id);
    }
...
Bugzilla::Extension::Testopia::TestRun - Testopia Test Run object
...
use Bugzilla::Extension::Testopia::TestRun;
 ...
 $run = Bugzilla::Extension::Testopia::TestRun->new($run_id);
 $run = Bugzilla::Extension::Testopia::TestRun->new(\%run_hash);

...
Bugzilla::Extension::Testopia::Search - A module to support searches in Testopis

=head1 DESCRIPTION

Bugzilla::Extension::Testopia::Search is based heavilly on Bugzilla::Search. It takes a 
...
=head1 SYNOPSIS
 ...
 $search = Bugzilla::Extension::Testopia::Search($cgi);

=cut

package Bugzilla::Extension::Testopia::Search;
...
sub init {
    ...
    if ($cgi->param('report_type') && $cgi->param('report_type') eq 'missing'){
        ...
        foreach my $p (@plan_ids){
            my $plan = Bugzilla::Extension::Testopia::TestPlan->new(trim($p));
...
# $suppstring = String which is pasted into query containing all table names    
    ...
    elsif ($order eq 'plan_name'){
        push @supptables, "INNER JOIN test_case_plans ON test_case_runs.case_id = test_case_plans.case_id";
        push @supptables, "LEFT JOIN test_plans AS plan_names ON plan_names.plan_id = test_case_plans.plan_id";
        push @orderby, 'plan_names.name';
    }
    ...
    foreach my $profile ("author", "manager", "default_tester", 
        ...
        $t = "isnull" if ($cgi->param($profile) && $cgi->param($profile) eq '--unassigned--');
        if ($t eq "exact"  || $t eq '') {
            $t = "anyexact";
...
Bugzilla::Extension::Testopia::Table Bugzilla::Search

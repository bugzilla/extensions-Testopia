...
if ($login && $passwd){
    ...
    $se->open( 'page.cgi?id=tr_builds.html' );
...
sub test_list {
    ...
    $se->open( format_url($test) );
    $se->wait_for_page_to_load(TIMEOUT);
    ok( $se->is_text_present("{builds:["), "Unable to open 'page.cgi?id=tr_builds.html'" );
#    $se->pause(3000);
...
sub test_add {
    ...
    $se->open( format_url($test) );
    $se->wait_for_page_to_load(TIMEOUT);
    ok( $se->is_text_present('success: true'), "'action=add' failed for page.cgi?id=tr_builds.html" );
...
sub test_edit {
    ...
    $se->open( format_url($test) );
    $se->wait_for_page_to_load(TIMEOUT);
    ok( $se->is_text_present("success: true"), "'action=edit' failed for page.cgi?id=tr_builds.html" );
